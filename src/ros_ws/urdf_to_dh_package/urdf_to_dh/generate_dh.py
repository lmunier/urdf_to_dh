...
class GenerateDhParams(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        default_urdf = os.path.join(os.getcwd(), "urdf_to_dh_package/urdf/random.urdf")
        self.rot_ref = np.eye(3)
        self.h_array = None
        self.p_array = None
        ...
        self.declare_parameter(
            "zero_pos", False, ParameterDescriptor(type=ParameterType.PARAMETER_BOOL)
        )

        self.nb_moving_joints = 0
        self.urdf_joints = {}
        self.urdf_links = {}
        self.carry_forward = {}
        ...
        self.urdf_file = (
            self.get_parameter("urdf_file").get_parameter_value().string_value
        )

        self.zero_pos = self.get_parameter("zero_pos").get_parameter_value().bool_value
    ...
    def __parse_links(self, urdf_root: ET.Element):
        ...
        for child in urdf_root:
            if child.tag == "link":
                ...
                link_node = AnyNode(
                    id=child.get("name"), parent=None, children=None, type="link"
                )
    ...
    def __parse_joints(self, urdf_root: ET.Element):
        ...
        for child in urdf_root:
            if child.tag == "joint":
                ...
                joint_node = AnyNode(
                    id=joint_name, parent=None, children=None, type="joint"
                )
    ...
    def __define_fixed_joint_axis(self):
        ...
        self.h_array = np.zeros((self.nb_moving_joints, 3))
        self.p_array = np.zeros((self.nb_moving_joints + 1, 3))
    ...
    def __get_joint_and_parent_axis(self, node: AnyNode) -> tuple:
        ...
        joint_axis = kh.normalize(self.urdf_joints[node.parent.id]["axis"])

        try:
            if self.urdf_joints[node.parent.parent.parent.id]["type"] in [
                "fixed",
                None,
            ]:
    ...
    def __get_common_normal(
        self,
        parent_joint_axis: np.ndarray,
        joint_axis_in_parent: np.ndarray,
        tf: np.ndarray,
    ) -> np.ndarray:
        ...
        """
        common_normal = kh.normalize(np.cross(parent_joint_axis, joint_axis_in_parent))
        ...
        if np.linalg.norm(common_normal) < EPSILON:
            ...
            if np.linalg.norm(np.cross(parent_joint_axis, tf[0:3, 3])) < EPSILON:
                common_normal = kh.inv_tf(tf)[0:3, 0:3] @ kh.normalize(
                    self.reference_axis
                )
            ...
            # Parallel revolute axis
                ...
                common_normal = kh.normalize(
                    np.cross(np.cross(parent_joint_axis, tf[0:3, 3]), parent_joint_axis)
                )
    ...
    def compute_dh_params(self):
        ...
        for n in LevelOrderIter(self.root_link):
            if n.type == "link" and n.parent is not None:
                ...
                self.carry_forward[n.id] = {"d": 0, "theta": 0}

                joint_axis, parent_joint_axis = self.__get_joint_and_parent_axis(n)
                ...
                d_val += self.carry_forward.get(n.parent.parent.id, {}).get("d", 0)

                # Round and adjust values
                if self.zero_pos:
                    alpha_val, d_val = self.__adjust_dh_sign(alpha_val, d_val)

                d_val = 0 if np.abs(d_val) < EPSILON else np.round(d_val, PRECISION)
                a_val = 0 if np.abs(a_val) < EPSILON else np.round(a_val, PRECISION)

                origin_transformed = kh.get_single_transformation(
                    d_val, theta_val, a_val, alpha_val
                ) @ np.array([0, 0, 0, 1])
                d_remaining = np.dot(
                    tf[0:3, 3] - origin_transformed[0:3], joint_axis_in_parent
                )
                ...
                print(
                    f"origin transformed : {origin_transformed[:3]}, actual recorded : {tf[0:3, 3]}, "
                ...
                print(tf)
                print(
                    f"joint axis: {joint_axis}, joint axis in parent : {joint_axis_in_parent}"
                )

                if self.carry_forward[n.id]["d"] != 0:
                    print(
                        kh.get_single_transformation(d_val, theta_val, a_val, alpha_val)
                    )

                self.urdf_links[n.id]["dh_found"] = True
                self.urdf_joints[n.id] = np.array([d_val, theta_val, a_val, alpha_val])

                if self.urdf_joints[n.parent.id]["type"] not in [None, "fixed"]:
                    print(f"xyz: {self.urdf_joints[n.parent.id]['xyz'], tf[0:3, 3]}")
                    print(f"rpy: {self.urdf_joints[n.parent.id]['rpy']}")

                    self.rot_ref = self.rot_ref @ tf[0:3, 0:3]
                    self.h_array[i_joint] = self.rot_ref @ joint_axis
                    self.p_array[i_joint + 1] = tf[0:3, 3]
    ...
    def display_dh_params(self):
        ...
        for urdf_node in LevelOrderIter(self.root_link):
            if urdf_node.type == "link" and urdf_node.parent is not None:
                robot_dh_params.append(
                    [urdf_node.parent.id, urdf_node.parent.parent.id, urdf_node.id]
                    + self.urdf_joints[urdf_node.id].tolist()
                )

        pd_frame = pd.DataFrame(
            robot_dh_params,
            columns=["joint", "parent", "child", "d", "theta", "r", "alpha"],
        )
        ...
        save_dir = os.path.join(os.getcwd(), "dh_parameters")
        ...
        print("\nh_array:\n", np.array2string(self.h_array, separator=","))
        print("\np_array:\n", np.array2string(self.p_array, separator=","), "\n")
